...
这意味着 SVM 要在约束条件 $y_i(w^Tx_i + b) \geq 1 (i=1,2,\cdots,m)$ 下，最大化 $\frac{1}{\Vert w \Vert_2}$。可以看出，这与感知机的优化方式不同，感知机是固定分母优化分子，而 SVM 是固定分子优化分母，同时加上了支持向量的限制。
...
在软间隔最大化时，则稍微复杂一些，因为需要对每个样本 $(x_i,y_i)$ 引入松弛变量 $\xi_i$。下图表示软间隔最大化时支持向量的情况，第 $i$ 个点到对应类别支持向量的距离为 $\frac{\xi_i}{\Vert w \Vert_2}$。根据软间隔最大化时 KKT 条件中的对偶互补条件 $\alpha_{i}^*(y_i(w^Tx_i + b) - 1 + \xi_i^*) = 0$ 有：
...
$$ s.t. \text{ } \vert y_i - w \cdot \phi(x_i ) -b\vert  \leq \epsilon (i =1,2,\cdots,m)$$
